/**
 * Complete Production Database Seeding Script - UPDATED
 * 
 * This script matches the ACTUAL production database structure.
 * It will add answer_choices to all 387 checklist items in production.
 * 
 * Collection: sectionchecklists (not section_checklists!)
 * Field to match: text (not title!)
 */
...
const SectionChecklistSchema = new mongoose.Schema({
  ...
  text: String, // Production uses 'text' not 'title'!
  value: String,
  comment: String,
  type: String,
  tab: String,
  ...
  order_index: Number
}, { collection: 'sectionchecklists', timestamps: true }); // Note: 'sectionchecklists' not 'section_checklists'!

const SectionChecklist = mongoose.model('SectionChecklist', SectionChecklistSchema);

// Complete mapping of answer choices - based on actual production 'text' field
const answerChoicesMap = {
  ...
  // Section 2 - Orientation / Shutoffs (3 items with answer choices)
  ...
  // Section 6 - Roof
  'General: Inspection Method': ['From Ground', 'From Ladder', 'Walked on Roof', 'From Drone', 'Binoculars', 'From Attic', 'From Window', 'Partial Access', 'Limited View', 'Not Accessible'],
  ...
  'General: Location': ['Attic', 'Garage', 'Closet', 'Basement', 'Crawl Space', 'Utility Room', 'Laundry Room', 'Hallway', 'Multiple Locations'],
  'General: Energy Source': ['Natural Gas', 'Propane', 'Electric', 'Oil', 'Wood'],

  // Section 11 - Water Heater
  ...
  'General: Location': ['Garage', 'Basement', 'Utility Room', 'Closet', 'Attic', 'Crawl Space', 'Exterior', 'Laundry Room', 'Kitchen', 'Bathroom', 'Hallway', 'Living Space', 'Mechanical Room', 'Furnace Room', 'Under Stairs', 'Pantry', 'Master Bedroom', 'Hall Closet', 'Entry Closet', 'Linen Closet', 'Storage Room', 'Carport', 'Patio', 'Porch', 'Multiple Locations', 'Unknown'],
...
async function seedProduction() {
  try {
    ...
    const answer = await new Promise(resolve => {
      rl.question('⚠️  Is this your PRODUCTION database? (yes/no):', resolve);
    });
    rl.close();
